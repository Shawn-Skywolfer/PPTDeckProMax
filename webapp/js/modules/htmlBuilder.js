...
class HTMLBuilder {
  async buildHTML(project) {
    try {
      ...
      const html = this._generateDeckHTML(pages, project, theme);
  ...
  _generateDeckHTML(pages, project, theme) {
...
<article class="slide" id="slide-${idx + 1}" data-index="${idx + 1}" style="display:none;">
  <div class="slide-inner" style="background:${isFirst ? primary : bg};color:${isFirst ? '#fff' : textDark};">
    <div class="slide-header">
      ...
      <span class="slide-number" style="color:${isFirst ? 'rgba(255,255,255,0.6)' : '#94a3b8'};">${String(idx + 1).padStart(2, '0')}</span>
    </div>
